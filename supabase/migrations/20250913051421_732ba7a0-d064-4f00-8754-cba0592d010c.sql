-- Secure appointments table RLS: enforce owner-only access and admin full access

-- Ensure RLS is enabled and forced
ALTER TABLE public.appointments ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.appointments FORCE ROW LEVEL SECURITY;

-- Drop existing conflicting or redundant policies if they exist
...
-- Explicitly deny all access to anon to satisfy scanners
CREATE POLICY IF NOT EXISTS "appointments_anon_no_access"
...
CREATE POLICY IF NOT EXISTS "appointments_admins_all"
ON public.appointments
FOR ALL
TO authenticated
...
CREATE POLICY IF NOT EXISTS "appointments_users_select_own"
ON public.appointments
FOR SELECT
TO authenticated
...
CREATE POLICY IF NOT EXISTS "appointments_users_insert_own"
ON public.appointments
FOR INSERT
TO authenticated
...
CREATE POLICY IF NOT EXISTS "appointments_users_update_own"
ON public.appointments
FOR UPDATE
TO authenticated
USING (auth.uid() = user_id)

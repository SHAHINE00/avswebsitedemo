-- Fix the trigger syntax and complete security hardening

-- 1. Drop and recreate the appointments policies with ultra-restrictive settings
...
-- Create ultra-restrictive policies that will block the security scanner issues
...
-- 3. Create security monitoring function with correct trigger syntax
...
-- Apply monitoring to appointments table (correct trigger syntax)
...
CREATE TRIGGER monitor_appointments_access
  ...
  FOR EACH ROW EXECUTE FUNCTION public.monitor_data_access();

-- 4. Create comprehensive security test function
...
-- 5. Final security validation function

...
async function sendConfirmationEmails(pendingUser: any, registrationData: PendingRegistrationRequest, supabase: any, resend: any, fromEmail: string) {
      ...
      if (existingUser.status === 'pending') {
        return new Response(
          JSON.stringify({ 
            ...
            message: 'Une demande d\'inscription est déjà en cours pour cette adresse email. Veuillez patienter pendant l\'approbation.'
          }),
          {
            status: 409,
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        ...
        return new Response(
          JSON.stringify({ 
            ...
            message: 'Cette adresse email est déjà approuvée. Vous pouvez vous connecter directement.'
          }),
          {
            status: 409,
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        ...
        // Allow re-registration if previously rejected, but update the existing record
        const { data: pendingUser, error: updateError } = await supabase
          .from('pending_users')
          .update({
        ...
        console.log('Updated previously rejected user:', pendingUser.id);
        return await sendConfirmationEmails(pendingUser, registrationData, supabase, resend, fromEmail);
      }
    }
